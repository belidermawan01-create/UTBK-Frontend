...
**Deskripsi:** Buat tryout baru (status DRAFT). Secara otomatis menginisialisasi subtes TPS (urutan 1) dan TKA_SAINTEK (urutan 2). Tryout yang dibuat akan dikaitkan dengan admin yang sedang login, sehingga admin tersebut dapat melihat tryout tersebut lewat endpoint list/detail.  
**Auth required:** Ya  
**Role required:** `ADMIN`
...
**Deskripsi:** Daftar tryout untuk siswa dan admin. Untuk siswa, hanya tryout berstatus PUBLISHED dan ONGOING yang ditampilkan. Untuk admin, endpoint menampilkan tryout publik (PUBLISHED/ONGOING) beserta tryout yang dibuat oleh admin yang sedang login, termasuk tryout berstatus DRAFT. Sistem secara otomatis mengupdate status tryout sebelum dikembalikan.  
**Auth required:** Ya  
**Role required:** `SISWA` atau `ADMIN`
...
`403 Forbidden` — role pengakses bukan SISWA atau ADMIN:

```json
{
  "message": "Akses ditolak. Diperlukan role: SISWA atau ADMIN"
}
```
...
**Deskripsi:** Detail tryout untuk siswa dan admin. Siswa hanya dapat melihat tryout yang berstatus ONGOING, sedangkan admin dapat melihat tryout milik sendiri tanpa memandang status.  
**Auth required:** Ya  
**Role required:** `SISWA` atau `ADMIN`
...
`403 Forbidden` — akses ditolak karena bukan siswa yang berhak melihat tryout ini atau admin mencoba melihat tryout milik admin lain:
