...
export default function Dashboard() {
  ...
  const avgSkor = selesai.length
    ? Math.round(
        selesai.reduce((s, r) => s + (r.skor || 0), 0) / selesai.length,
      )
    : 0;
  const best = selesai.length
    ? Math.max(...selesai.map((r) => r.skor || 0))
    : 0;
      ...
      <div className="container">
        <div className="page-header">
          <div>
            ...
            <p className="page-sub">
              Halo, <strong>{user?.email?.split("@")[0]}</strong>! Yuk lanjut
              belajar <Rocket size={20} />
            </p>
          </div>
          <Link to="/latihan" className="btn btn-primary">
            + Mulai Latihan
          </Link>
        </div>
        ...
        <div className="mapel-grid">
          {(user?.role?.toLowerCase() === "admin" ||
            ...
            >
              <div className="mapel-name">
                <Settings size={20} /> Dashboard Admin
              </div>
              <div className="mapel-arrow" style={{ color: "var(--primary)" }}>
                →
              </div>
            </Link>
          )}
          {Object.entries(MAPEL_LABEL).map(([key, label]) => (
            <Link
              key={key}
              to={`/latihan?mapel=${key}`}
              className={`mapel-card mapel-${MAPEL_COLOR[key]}`}
            >
          ...
          >
            <div className="mapel-name">
              <ClipboardList size={20} /> Tryout UTBK
            </div>
            <div className="mapel-arrow">→</div>
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>
              Belum ada sesi latihan. <Link to="/latihan">Mulai sekarang!</Link>
            </p>
          </div>
        ) : (
          <div className="session-list">
            {riwayat.slice(0, 5).map((r) => (
              <Link
                key={r.id}
                to={r.selesai ? `/hasil/${r.id}` : "#"}
                className="session-item"
              >
                <div className="session-info">
                  <span
                    className={`mapel-badge mapel-badge-${MAPEL_COLOR[r.mapel]}`}
                  >
                    {MAPEL_LABEL[r.mapel]}
                  </span>
                ...
                <div className="session-status">
                  {r.selesai ? (
                    <ScoreRing skor={r.skor} />
                  ) : (
                    <span className="badge-ongoing">Berlangsung</span>
                  )}
                </div>
              </Link>
            ))}
